...
{
    if (argc < 3) {
        printf("Not enough inputs!\nUsage : ./NNSC -i img_name [-outm output_map_name] [-outb output_border_name]  [-k superpixel_nbr] [-m compactness] [-Kpm knn_by_PM] [-patch_w patch_size]\n");
        return -1;
    }
    ...
    int Kpm     = cimg_option("-Kpm", 4, "Number of knn computed by PatchMatch");
    int patch_w = cimg_option("-pacth_w", 3, "Half patch size");
